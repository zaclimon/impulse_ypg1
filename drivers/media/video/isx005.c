...
/*
 * Stop_af_operation is used to cancel the operation
 * while doing or even before it has started
 */
static volatile int stop_af_operation;
//static volatile int af_status_check;

/*
 * Here we store the status of AF;
 * 0 --> AF not started
 * 1 --> AF started
 * 2 --> AF operation finished
 */
...
void isx005_flash_set_first(struct v4l2_subdev *sd)
{
...
#ifdef CONFIG_LOAD_FILE
	isx005_i2c_write(sd, isx005_flash_first, sizeof(isx005_flash_first)
		/ sizeof(isx005_flash_first[0]), "isx005_flash_first");
#else
	...
	// Set AE Hi-Speed mode (HALF_AE_CTRL(bit0)=1&CAP_AE_CTRL(bit1-2)=2)
	aectrl = 5;
	ret = isx005_i2c_write_multi(client, CAP_HALF_AE_CTRL, aectrl,
		sizeof(unsigned char));
	// Set AWB Hi-Speed mode
	awbctrl = 1;
	ret = isx005_i2c_write_multi(client, HALF_AWB_CTRL, awbctrl,
		sizeof(unsigned char));
}
...
void isx005_flash_set_second(struct v4l2_subdev *sd)
{
...
#ifdef CONFIG_LOAD_FILE
	unsigned short ae_offsetval_value =
		isx005_regs_ae_offsetval_value("AE_OFFSETVAL");
	...
	// Set AE Gain offset
	ret = isx005_i2c_write_multi(client, CAP_GAINOFFSET, aeoffset,
		sizeof(short unsigned int));
	dev_err(&client->dev, "%s: AESCL_NOW : %d, ERRSCL_NOW : %d, aediff : %d, "
	...
	// Set AWB offset
	smod = 2; //100813 1->2
	ret = isx005_i2c_write_multi(client, CONT_SHIFT, smod,
		sizeof(unsigned char));
}
...
void isx005_flash_end_capture(struct v4l2_subdev *sd)
{
	...
	isx005_i2c_write_multi(client, CAP_HALF_AE_CTRL, temp,
		sizeof(unsigned char));
	isx005_i2c_write_multi(client, HALF_AWB_CTRL, temp, sizeof(unsigned char));

	isx005_i2c_write_multi(client, CAP_GAINOFFSET, tempi,
		sizeof(short unsigned int));
	isx005_i2c_write_multi(client, CAP_HALF_AE_CTRL, temp,
		sizeof(unsigned char));
	isx005_i2c_write_multi(client, STB_CONT_SHIFT_R, tempi,
		sizeof(short unsigned int));
	isx005_i2c_write_multi(client, STB_CONT_SHIFT_B, tempi,
		sizeof(short unsigned int));
				...
				unsigned short subaddr, unsigned long *data)
{
	...
	if (unlikely(err < 0)) {
		dev_err(&client->dev, "%s: %d reg read fail\n", __func__, __LINE__);
		return -EIO;
	}
	...
	err = i2c_transfer(client->adapter, &msg, 1);
	if (unlikely(err < 0)) {
		dev_err(&client->dev, "%s: %d regi read fail\n", __func__, __LINE__);
		return -EIO;
	}
 ...
 * Returns 0 on success, <0 on error
 */
static inline int isx005_i2c_write_multi(struct i2c_client *client,
				...
				int size, char *name)
{
	...
	for (i = 0; i < size; i++) {
		err = isx005_i2c_write_multi(client, regs[i].subaddr, regs[i].value,
			regs[i].len);
		if (unlikely(err < 0)) {
	...
	*(len_buf + 1) = '\0';
	...
	while (1) {
		...
		if (reg != NULL) {
			...
			if (addr == 0xdddd) {
				msleep(value);
				isx005_msg(&client->dev,"d: 0x%04x, v: 0x%04x, l: 0x%01x\n",
					addr, value, len);
			} else {
...
static int isx005_set_frame_rate(struct v4l2_subdev *sd,
...
static int isx005_set_dzoom(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	switch (ctrl->value) {
		...
		case 20:
		case 30:
			err = isx005_i2c_write(sd, isx005_Zoom_00,
				sizeof(isx005_Zoom_00) / sizeof(isx005_Zoom_00[0]),
				"isx005_Zoom_00");
			break;
		case 1:
			err = isx005_i2c_write(sd, isx005_Zoom_01,
				sizeof(isx005_Zoom_01) / sizeof(isx005_Zoom_01[0]),
				"isx005_Zoom_01");
			break;
		case 2:
			err = isx005_i2c_write(sd, isx005_Zoom_02,
				sizeof(isx005_Zoom_02) / sizeof(isx005_Zoom_02[0]),
				"isx005_Zoom_02");
			break;
		case 3:
			err = isx005_i2c_write(sd, isx005_Zoom_03,
				sizeof(isx005_Zoom_03) / sizeof(isx005_Zoom_03[0]),
				"isx005_Zoom_03");
			break;
		case 4:
			err = isx005_i2c_write(sd, isx005_Zoom_04,
				sizeof(isx005_Zoom_04) / sizeof(isx005_Zoom_04[0]),
				"isx005_Zoom_04");
			break;
		case 5:
			err = isx005_i2c_write(sd, isx005_Zoom_05,
				sizeof(isx005_Zoom_05) / sizeof(isx005_Zoom_05[0]),
				"isx005_Zoom_05");
			break;
		case 6:
			err = isx005_i2c_write(sd, isx005_Zoom_06,
				sizeof(isx005_Zoom_06) / sizeof(isx005_Zoom_06[0]),
				"isx005_Zoom_06");
			break;
		case 7:
			err = isx005_i2c_write(sd, isx005_Zoom_07,
				sizeof(isx005_Zoom_07) / sizeof(isx005_Zoom_07[0]),
				"isx005_Zoom_07");
			break;
		default:
...
static int isx005_set_preview_size(struct v4l2_subdev *sd)
{
	...
	switch (index) {
		...
		default:
			 ...
			 */
			dev_err(&client->dev, "Preview resoution as SVGA capture mode\n");
			break;
	}
	...
	// Moni_Refresh
	err = isx005_i2c_write_multi(client, 0x0012, 0x0001, 1);
	...
	do {
		...
		msleep(1);
		if (timeout_cnt > 1000) {
			dev_err(&client->dev, "%s: Entering moniter mode "
				"timed out \n", __func__);
			break;
		}
...
static int isx005_set_preview_start(struct v4l2_subdev *sd)
{
	...
	if (state->runmode == ISX005_RUNMODE_IDLE) {
		err = isx005_i2c_write(sd, isx005_cap_to_prev,
			sizeof(isx005_cap_to_prev) / sizeof(isx005_cap_to_prev[0]),
			"isx005_cap_to_prev");

		if (err < 0) {
			dev_err(&client->dev, "%s: failed: Could not change capture "
				"to preview\n", __func__);
			return -EIO;
		}
	}
	...
	if (state->set_vhflip == 1) {
		err = isx005_i2c_write(sd, isx005_vhflip_on,
			sizeof(isx005_vhflip_on) / sizeof(isx005_vhflip_on[0]),
			"isx005_vhflip_on");
	} else {
		err = isx005_i2c_write(sd, isx005_vhflip_off,
			sizeof(isx005_vhflip_off) / sizeof(isx005_vhflip_off[0]),
			"isx005_vhflip_off");
	}
	...
	if (state->check_dataline) // Output Test Pattern
		...
		if (camera_init == 0) { // 100818
			...
			do {
				...
				if (timeout_cnt > 300) {
					dev_err(&client->dev, "%s: Entering AE stabilization "
						"timed out \n", __func__);
					break;
				}
			} while (!(read_value==0));
...
static int isx005_LowLightCondition_Off(struct v4l2_subdev *sd)
{
	...
	if (gLowLight == 1) {
		...
		if (gCurrentScene == SCENE_MODE_NIGHTSHOT) {
			isx005_msg(&client->dev,"SCENE_MODE_NIGHTSHOT --- "
				"isx005_Night_Mode_Off: start \n");
			err = isx005_i2c_write(sd,
				isx005_Night_Mode_Off,
				sizeof(isx005_Night_Mode_Off)
				/ sizeof(isx005_Night_Mode_Off[0]),
				"isx005_Night_Mode_Off");
		}
		else
		{
			isx005_msg(&client->dev,"Not Night mode --- "
				"isx005_Low_Cap_Off: start \n");
...
static int isx005_get_LowLightCondition_flash (struct v4l2_subdev *sd,
				int *Result)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
...
static int isx005_get_LowLightCondition_iso (struct v4l2_subdev *sd,
				int *Result)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	...
	if (gIsoCondition == 2) {
		...
		if (read_value >= 0x182B8)
			*Result = 1; //gLowLight
	} else if (gIsoCondition == 3) {
		//ISO 200
		if (read_value >= 0x10B80)
			*Result = 1; //gLowLight
	} else if (gIsoCondition == 4) {
		//ISO 400
				...
				struct v4l2_control *ctrl)
{
	...
	if ((read_value & 0xFF) != 0x01) {
		...
		switch (flash_mode) {
			case FLASHMODE_AUTO:
				if (gLowLight_flash) {
					...
					isx005_i2c_write(sd, isx005_half_release_with_flash,
						sizeof(isx005_half_release_with_flash)
						/ sizeof(isx005_half_release_with_flash[0]),
						"isx005_half_release_with_flash");
				}
				break;
		...
		do {
			...
			msleep(1);
			if (timeout_cnt > 1000) {
				dev_err(&client->dev, "%s: Entering Half release mode "
					"timed out\n", __func__);
				break;
			}
		} while (!(read_value & 0x02));
	...
	if (read_value == 0x01) {
		/*CAPNUM is setted 3. default value is 2. */
		isx005_i2c_write_multi(client, 0x0014, 0x0003, 1);
	...
	if (gLowLight && !flash_check) {
		...
		if (gCurrentScene == SCENE_MODE_NIGHTSHOT) {
			...
			err = isx005_i2c_write(sd,
				...
				sizeof(isx005_Night_Mode_On)
				/ sizeof(isx005_Night_Mode_On[0]),
				"isx005_Night_Mode_On");
			if (err < 0) {
	...
	if (gCurrentScene != SCENE_MODE_NIGHTSHOT) {
		//when scene mode is not nightshot
		switch (flash_mode) {
			case FLASHMODE_AUTO:
				//if lowlight
				if (gLowLight_flash) {
...
static int isx005_change_scene_mode(struct v4l2_subdev *sd,
				struct v4l2_control *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	...
	switch (ctrl->value) {
		...
		case SCENE_MODE_NONE:
			err = isx005_i2c_write(sd,
				...
				sizeof(isx005_Scene_Default)
				/ sizeof(isx005_Scene_Default[0]),
				"isx005_Scene_Default");
			break;
		...
		case SCENE_MODE_PORTRAIT:
			err = isx005_i2c_write(sd,
				...
				sizeof(isx005_Scene_Portrait)
				/ sizeof(isx005_Scene_Portrait[0]),
				"isx005_Scene_Portrait");
			break;
		...
		case SCENE_MODE_NIGHTSHOT:
			err = isx005_i2c_write(sd,
				...
				sizeof(isx005_Scene_Nightshot)
				/ sizeof(isx005_Scene_Nightshot[0]),
				"isx005_Scene_Nightshot");
			break;
		...
		case SCENE_MODE_BACK_LIGHT:
			err = isx005_i2c_write(sd,
				...
				sizeof(isx005_Scene_Backlight)
				/ sizeof(isx005_Scene_Backlight[0]),
				"isx005_Scene_Backlight");
			break;
		...
		case SCENE_MODE_LANDSCAPE:
			err = isx005_i2c_write(sd,
				...
				sizeof(isx005_Scene_Landscape)
				/ sizeof(isx005_Scene_Landscape[0]),
				"isx005_Scene_Landscape");
			break;
		...
		case SCENE_MODE_SPORTS:
			err = isx005_i2c_write(sd,
				isx005_Scene_Sports,
				sizeof(isx005_Scene_Sports)
				/ sizeof(isx005_Scene_Sports[0]),
				"isx005_Scene_Sports");
			break;
		...
		case SCENE_MODE_PARTY_INDOOR:
			err = isx005_i2c_write(sd,
				...
				sizeof(isx005_Scene_Party_Indoor)
				/ sizeof(isx005_Scene_Party_Indoor[0]),
				"isx005_Scene_Party_Indoor");
			break;
		...
		case SCENE_MODE_BEACH_SNOW:
			err = isx005_i2c_write(sd,
				...
				sizeof(isx005_Scene_Beach_Snow)
				/ sizeof(isx005_Scene_Beach_Snow[0]),
				"isx005_Scene_Beach_Snow");
			break;
		...
		case SCENE_MODE_SUNSET:
			err = isx005_i2c_write(sd,
				isx005_Scene_Sunset,
				sizeof(isx005_Scene_Sunset)
				/ sizeof(isx005_Scene_Sunset[0]),
				"isx005_Scene_Sunset");
			break;
		...
		case SCENE_MODE_DUST_DAWN:
			err = isx005_i2c_write(sd,
				...
				sizeof(isx005_Scene_Duskdawn)
				/ sizeof(isx005_Scene_Duskdawn[0]),
				"isx005_Scene_Duskdawn");
			break;
		...
		case SCENE_MODE_FALL_COLOR:
			err = isx005_i2c_write(sd,
				...
				sizeof(isx005_Scene_Fall_Color)
				/ sizeof(isx005_Scene_Fall_Color[0]),
				"isx005_Scene_Fall_Color");
			break;
		...
		case SCENE_MODE_FIREWORKS:
			err = isx005_i2c_write(sd,
				...
				sizeof(isx005_Scene_Fireworks)
				/ sizeof(isx005_Scene_Fireworks[0]),
				"isx005_Scene_Fireworks");
			break;
		...
		case SCENE_MODE_TEXT:
			err = isx005_i2c_write(sd,
				isx005_Scene_Text,
				sizeof(isx005_Scene_Text)
				/ sizeof(isx005_Scene_Text[0]),
				"isx005_Scene_Text");
			break;
		...
		case SCENE_MODE_CANDLE_LIGHT:
			err = isx005_i2c_write(sd,
				...
				sizeof(isx005_Scene_Candle_Light)
				/ sizeof(isx005_Scene_Candle_Light[0]),
				"isx005_Scene_Candle_Light");
			break;
...
static int isx005_set_effect(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	switch (ctrl->value) {
		...
		case IMAGE_EFFECT_SHARPEN:
			err = isx005_i2c_write(sd, isx005_Effect_Normal,
				sizeof(isx005_Effect_Normal)
				/ sizeof(isx005_Effect_Normal[0]),
				"isx005_Effect_Normal");
			break;

		case IMAGE_EFFECT_BNW:
			err = isx005_i2c_write(sd, isx005_Effect_Black_White,
				sizeof(isx005_Effect_Black_White)
				/ sizeof(isx005_Effect_Black_White[0]),
				"isx005_Effect_Black_White");
			break;

		case IMAGE_EFFECT_SEPIA:
			err = isx005_i2c_write(sd, isx005_Effect_Sepia,
				sizeof(isx005_Effect_Sepia)
				/ sizeof(isx005_Effect_Sepia[0]),
				"isx005_Effect_Sepia");
			break;

		case IMAGE_EFFECT_NEGATIVE:
			err = isx005_i2c_write(sd, isx005_Effect_Negative,
				sizeof(isx005_Effect_Negative)
				/ sizeof(isx005_Effect_Negative[0]),
...
static int isx005_set_saturation(struct v4l2_subdev *sd,
				struct v4l2_control *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	...
	switch (ctrl->value) {
		...
		case SATURATION_MINUS_2:
			err = isx005_i2c_write(sd,
				...
				sizeof(isx005_Saturation_Minus_2)
				/ sizeof(isx005_Saturation_Minus_2[0]),
				"isx005_Saturation_Minus_2");
			break;
		...
		case SATURATION_MINUS_1:
			err = isx005_i2c_write(sd,
				...
				sizeof(isx005_Saturation_Minus_1)
				/ sizeof(isx005_Saturation_Minus_1[0]),
				"isx005_Saturation_Minus_1");
			break;
		...
		case SATURATION_DEFAULT:
			err = isx005_i2c_write(sd,
				...
				sizeof(isx005_Saturation_Default)
				/ sizeof(isx005_Saturation_Default[0]),
				"isx005_Saturation_Default");
			break;
		...
		case SATURATION_PLUS_1:
			err = isx005_i2c_write(sd,
				...
				sizeof(isx005_Saturation_Plus_1)
				/ sizeof(isx005_Saturation_Plus_1[0]),
				"isx005_Saturation_Plus_1");
			break;
		...
		case SATURATION_PLUS_2:
			err = isx005_i2c_write(sd,
				...
				sizeof(isx005_Saturation_Plus_2)
				/ sizeof(isx005_Saturation_Plus_2[0]),
				"isx005_Saturation_Plus_2");
			break;
...
static int isx005_set_contrast(struct v4l2_subdev *sd,
				struct v4l2_control *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	...
	switch (ctrl->value) {
		...
		case CONTRAST_MINUS_2:
			err = isx005_i2c_write(sd,
				...
				sizeof(isx005_Contrast_Minus_2)
				/ sizeof(isx005_Contrast_Minus_2[0]),
				"isx005_Contrast_Minus_2");
			break;
		...
		case CONTRAST_MINUS_1:
			err = isx005_i2c_write(sd,
				...
				sizeof(isx005_Contrast_Minus_1)
				/ sizeof(isx005_Contrast_Minus_1[0]),
				"isx005_Contrast_Minus_1");
			break;
		...
		case CONTRAST_DEFAULT:
			err = isx005_i2c_write(sd,
				...
				sizeof(isx005_Contrast_Default)
				/ sizeof(isx005_Contrast_Default[0]),
				"isx005_Contrast_Default");
			break;
		...
		case CONTRAST_PLUS_1:
			err = isx005_i2c_write(sd,
				...
				sizeof(isx005_Contrast_Plus_1)
				/ sizeof(isx005_Contrast_Plus_1[0]),
				"isx005_Contrast_Plus_1");
			break;
		...
		case CONTRAST_PLUS_2:
			err = isx005_i2c_write(sd,
				...
				sizeof(isx005_Contrast_Plus_2)
				/ sizeof(isx005_Contrast_Plus_2[0]),
				"isx005_Contrast_Plus_2");
			break;
...
static int isx005_set_sharpness(struct v4l2_subdev *sd,
				struct v4l2_control *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	...
	switch (ctrl->value) {
		...
		case SHARPNESS_MINUS_2:
			err = isx005_i2c_write(sd,
				...
				sizeof(isx005_Sharpness_Minus_2)
				/ sizeof(isx005_Sharpness_Minus_2[0]),
				"isx005_Sharpness_Minus_2");
			break;
		...
		case SHARPNESS_MINUS_1:
			err = isx005_i2c_write(sd,
				...
				sizeof(isx005_Sharpness_Minus_1)
				/ sizeof(isx005_Sharpness_Minus_1[0]),
				"isx005_Sharpness_Minus_1");
			break;
		...
		case SHARPNESS_DEFAULT:
			err = isx005_i2c_write(sd,
				...
				sizeof(isx005_Sharpness_Default)
				/ sizeof(isx005_Sharpness_Default[0]),
				"isx005_Sharpness_Default");
			break;
		...
		case SHARPNESS_PLUS_1:
			err = isx005_i2c_write(sd,
				...
				sizeof(isx005_Sharpness_Plus_1)
				/ sizeof(isx005_Sharpness_Plus_1[0]),
				"isx005_Sharpness_Plus_1");
			break;
		...
		case SHARPNESS_PLUS_2:
			err = isx005_i2c_write(sd,
				...
				sizeof(isx005_Sharpness_Plus_2)
				/ sizeof(isx005_Sharpness_Plus_2[0]),
				"isx005_Sharpness_Plus_2");
			break;
...
/*Camcorder fix fps*/
static int isx005_set_sensor_mode(struct v4l2_subdev *sd,
				struct v4l2_control *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
...
static int isx005_set_focus_mode(struct v4l2_subdev *sd,
				struct v4l2_control *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	int err = 0;
	...
	switch (ctrl->value) {
		...
		case WHITE_BALANCE_FLUORESCENT:
			err = isx005_i2c_write(sd,
				...
				sizeof(isx005_WB_Fluorescent)
				/ sizeof(isx005_WB_Fluorescent[0]),
				"isx005_WB_Fluorescent");
			break;
	...
	switch (ctrl->value) {
		...
		case METERING_MATRIX:
			err = isx005_i2c_write(sd,
				...
				sizeof(isx005_Metering_Matrix)
				/ sizeof(isx005_Metering_Matrix[0]),
				"isx005_Metering_Matrix");
			break;
		...
		case METERING_CENTER:
			err = isx005_i2c_write(sd,
				...
				sizeof(isx005_Metering_Center)
				/ sizeof(isx005_Metering_Center[0]),
				"isx005_Metering_Center");
			break;
		...
		case METERING_SPOT:
			err = isx005_i2c_write(sd,
				...
				sizeof(isx005_Metering_Spot)
				/ sizeof(isx005_Metering_Spot[0]),
				"isx005_Metering_Spot");
			break;
...
static int isx005_set_auto_focus(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	if (ctrl->value == 1) {
		...
		if (af_mode == FOCUS_MODE_MACRO) {
			err = isx005_i2c_write(sd, isx005_AF_Return_Macro_pos,
				ISX005_AF_RETURN_MACRO_POS_REGS, "isx005_AF_Return_Macro_pos");
			...
			err = isx005_i2c_write(sd, isx005_AF_Return_Inf_pos,
				ISX005_AF_RETURN_INF_POS_REGS, "isx005_AF_Return_Inf_pos");
			if (err < 0) {
				dev_err(&client->dev, "%s: register write fail \n", __func__);
				return -EIO;
			}
		...
		err = isx005_i2c_write(sd, isx005_Single_AF_Start,
			ISX005_SINGLE_AF_START_REGS, "isx005_Single_AF_Start");
		...
		if (af_operation_status == 2) {
			...
			if (af_mode == FOCUS_MODE_MACRO) {
				err = isx005_i2c_write(sd, isx005_AF_Return_Macro_pos,
					ISX005_AF_RETURN_MACRO_POS_REGS,
					"isx005_AF_Return_Macro_pos");
...
static int isx005_get_auto_focus_status(struct v4l2_subdev *sd,
				struct v4l2_control *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	...
	if (stop_af_operation) {
		...
		err = isx005_i2c_write(sd, isx005_Single_AF_Off,
			ISX005_SINGLE_AF_OFF_REGS, "isx005_Single_AF_Off");
		...
		err = isx005_i2c_write(sd, isx005_Single_AF_Off, ISX005_SINGLE_AF_OFF_REGS,
			"isx005_Single_AF_Off");
...
#if defined(ISX005_TOUCH_AF)
static int isx005_set_touch_auto_focus(struct v4l2_subdev *sd,
				struct v4l2_control *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct isx005_state *state = to_state(sd);
	...
	if (ctrl->value == 0) {
		...
		// Actually To subtract window size(AF_OPD_HVALID) is divided in half,
		// real Hdelay is calculated.

		// AF_OPD_HDELAY = X position + 8(color processing margin)
		AF_OPD4_HDELAY = ((x * H_ratio) / 100) - (AF_OPD4_HVALID / 2) + 8;
		// AF_OPD_VDELAY = Y position
		AF_OPD4_VDELAY = ((y * V_ratio) / 100) - (AF_OPD4_VVALID / 2);
		// AF_OPD_HDELAY = X position + 8(color processing margin)
		AF_OPD5_HDELAY = ((x * H_ratio) / 100) - (AF_OPD5_HVALID / 2) + 8;
		// AF_OPD_VDELAY = Y position
		AF_OPD5_VDELAY = ((y * V_ratio) / 100) - (AF_OPD5_VVALID / 2);
		...
		// out of boundary... max.
		if (AF_OPD4_HDELAY > 2064 - AF_OPD4_HVALID - 8)
			AF_OPD4_HDELAY = 2064 - AF_OPD4_HVALID - 8;
		if (AF_OPD4_VDELAY > 1536 - AF_OPD4_VVALID)
			AF_OPD4_VDELAY = 1536 - AF_OPD4_VVALID;
		if (AF_OPD5_HDELAY > 2064 - AF_OPD5_HVALID - 8)
			AF_OPD5_HDELAY = 2064 - AF_OPD5_HVALID - 8;
		if (AF_OPD5_VDELAY > 1536 - AF_OPD5_VVALID)
			AF_OPD5_VDELAY = 1536 - AF_OPD5_VVALID;
		...
		// out of boundary... min.
		if (AE_ARB_OPD_HDELAY < 8)
			AE_ARB_OPD_HDELAY = 8;
		if (AE_ARB_OPD_VDELAY < 0)
			AE_ARB_OPD_VDELAY = 0;

		// out of boundary... max.
		if (AE_ARB_OPD_HDELAY > 2064 - AE_ARB_OPD_HVALID - 8)
			AE_ARB_OPD_HDELAY = 2064 - AE_ARB_OPD_HVALID - 8;
		if (AE_ARB_OPD_VDELAY > 1536 - AE_ARB_OPD_VVALID)
			AE_ARB_OPD_VDELAY = 1536 - AE_ARB_OPD_VVALID;
...
static int isx005_aeawb_unlock(struct v4l2_subdev *sd,
				struct v4l2_control *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
...
static int isx005_get_esd_int(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	if (err < 0) {
		dev_err(&client->dev, "Failed I2C communication for "
			"getting ESD information\n");
		ctrl->value = 0x01;
	} else {
		if (val == 0xFF) {
...
static int isx005_get_shutterspeed(struct v4l2_subdev *sd,
				struct v4l2_control *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
...
static int isx005_g_mbus_fmt(struct v4l2_subdev *sd,
...
static int isx005_get_framesize_index(struct v4l2_subdev *sd)
{
	...
	isx005_msg(&client->dev, "%s: Requested : %dx%d\n",
	...
	for (i = 0; i < (sizeof(isx005_framesize_list)
		...
		if (frmsize->width == state->pix.width
			...
			isx005_msg(&client->dev, "%s: Defined : %dx%d\n",
					__func__, state->pix.width, state->pix.height);
			return frmsize->index;
		}
	}

	isx005_msg(&client->dev, "%s: Resolution FAIL: %dx%d\n",
			__func__, state->pix.width, state->pix.height);

	/* If it fails, return the default value. */
	if (state->oprmode == ISX005_OPRMODE_IMAGE) {
		ret = ISX005_CAPTURE_W2MP;
		if (state->pix.height > 1300)
			ret = ISX005_CAPTURE_3MP;
	} else {
		ret = ISX005_PREVIEW_SVGA;
		if (state->pix.height > 600)
			return ISX005_PREVIEW_720P;
	}
 ...
 */
static int isx005_set_framesize_index(struct v4l2_subdev *sd,
				unsigned int index)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct isx005_state *state = to_state(sd);
 ...
 *
 * */
static int isx005_s_mbus_fmt(struct v4l2_subdev *sd,
				struct v4l2_mbus_framefmt *fmt)
{
	struct isx005_state *state = to_state(sd);
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	...
	if (state->oprmode == ISX005_OPRMODE_VIDEO) {
		if (framesize_index == ISX005_PREVIEW_720P)
			state->hd_preview_on = 1;
		else
			state->hd_preview_on = 0;
	}
...
static int isx005_try_mbus_fmt(struct v4l2_subdev *sd,
				struct v4l2_mbus_framefmt *fmt)
{
	int num_entries = 0;
	int i = 0;
...
static int isx005_s_parm(struct v4l2_subdev *sd, struct v4l2_streamparm *param)
{
	...
	* We will set the resolution and fps in the start
	* operation (preview/capture) call
	*/
...
static int isx005_s_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	switch (ctrl->id) {
		...
		case V4L2_CID_CAMERA_GPS_LATITUDE:
			dev_err(&client->dev, "%s: V4L2_CID_CAMERA_GPS_LATITUDE: "
				"not implemented\n", __func__);
			break;

		case V4L2_CID_CAMERA_GPS_LONGITUDE:
			dev_err(&client->dev, "%s: V4L2_CID_CAMERA_GPS_LONGITUDE: "
				"not implemented\n", __func__);
			break;

		case V4L2_CID_CAMERA_GPS_TIMESTAMP:
			dev_err(&client->dev, "%s: V4L2_CID_CAMERA_GPS_TIMESTAMP: "
				"not implemented\n", __func__);
			break;

		case V4L2_CID_CAMERA_GPS_ALTITUDE:
			dev_err(&client->dev, "%s: V4L2_CID_CAMERA_GPS_ALTITUDE: "
				"not implemented\n", __func__);
			break;
...
static int isx005_calibration(struct v4l2_subdev *sd)
{
	...
	if (valid_OPT == 1 || valid_OPT ==0) { //CASE 1 || CASE 2:
		...
		if (shading_index == 1) { // 01
			err = isx005_i2c_write(sd, isx005_shading_2,
			...
			err = isx005_i2c_write(sd, isx005_shading_3,
			...
			err = isx005_i2c_write(sd, isx005_shading_1,
	...
	isx005_msg(&client->dev,"%s [cam] CALIBRATION : END \n", __func__);
	return 1;
}
...
static int isx005_set_default_calibration(struct v4l2_subdev *sd)
{
	...
	err = isx005_i2c_write(sd,
		...
		sizeof(isx005_default_calibration)
		/ sizeof(isx005_default_calibration[0]),
		"isx005_default_calibration");
...
static int isx005_check_dataline_start(struct v4l2_subdev *sd)
{
	...
	for (i = 0; i <ISX005_PATTERN_ON_REGS; i++) {
		...
		err = isx005_i2c_write_multi(client, isx005_pattern_on[i].subaddr,
			isx005_pattern_on[i].value, isx005_pattern_on[i].len);
...
static int isx005_check_dataline_stop(struct v4l2_subdev *sd)
{
	...
	for (i = 0; i <ISX005_INIT_REGS; i++)
		err = isx005_i2c_write_multi(client, isx005_init_reg[i].subaddr,
			isx005_init_reg[i].value, isx005_init_reg[i].len);
	...
	for (i = 0; i <ISX005_INIT_IMAGETUNING_SETTING_REGS; i++)
		err = isx005_i2c_write_multi(client,
			isx005_init_image_tuning_setting[i].subaddr,
			isx005_init_image_tuning_setting[i].value,
			isx005_init_image_tuning_setting[i].len);
...
static int isx005_init(struct v4l2_subdev *sd, u32 val)
{
	...
	isx005_msg(&client->dev, "%s: isx005_init_image_tuning_setting~~~~~~~~~~\n",
		__func__);
	isx005_i2c_write(sd, isx005_init_image_tuning_setting,
		ISX005_INIT_IMAGETUNING_SETTING_REGS,
		"isx005_init_image_tuning_setting");
	isx005_msg(&client->dev,"isx005_init\n");
#else
	for (i = 0; i <ISX005_INIT_REGS; i++) {
...
extern ssize_t camtype_show(struct device *dev, struct device_attribute *attr,
	char *buf);
extern ssize_t camtype_store(struct device *dev, struct device_attribute *attr,
